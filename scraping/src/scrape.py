    ...
    for category_url in config.category_urls:
        ...
        while True:
            ...
            for ele in soup.find("div", attrs={"class": "mainBody"}).find("ul", attrs={"class": "articleList"}).find_all("li"):
                # リンクを修正する
                page_url = ele.find("a").get(
                    "href").replace("topics", "article")
                # すでにスクレイピング済みだったら
                ...
                # データ習得
                try:
                    article_category = "<SEP>".join([c.text for c in article_soup.find(
                        "nav", attrs={"class": "breadcrumbs"}).find_all("span", attrs={"itemprop": "name"})])
                except:
                    article_category = "None Category"
                try:
                    article_keyword = "<SEP>".join([k.text for k in article_soup.find(
                        "ul", attrs={"class": "articleHeadKeyword"}).find_all("a")])
                except:
                    article_keyword = "None Keyword"
                try:
                    article_date = article_soup.find(
                        "time", attrs={"class": "articleDate"}).text
                except:
                    article_date = "None Date"
                try:
                    article_vender = article_soup.find("p", attrs={"class": "articleVender"}).find(
                        "span", attrs={"itemprop": "name"}).text.strip(" ").strip("\n")
                except:
                    article_vender = "None Vender"
                try:
                    article_tweet_counts = article_soup.find("ul", attrs={"class": "socialBtn"}).find(
                        "div", attrs={"class": "tweet_counts"}).text
                except:
                    article_tweet_counts = 0
                try:
                    article_title = article_soup.find(
                        "h1", attrs={"class": "articleTtl"}).text
                except:
                    article_title = "None Title"
                try:
                    article_text = article_soup.find(
                        "span", attrs={"itemprop": "articleBody"}).text
                except:
                    article_text = "None Text"
                # データを一旦listに保存
                article_infos.append([page_url, article_date, article_title, article_text,
                                     article_category, article_keyword, article_tweet_counts, article_vender])
                print("{}の抽出完了".format(page_url))
                print("記事のタイトル: {0}, 記事の本文一部: {1}".format(
                    article_title, article_text[0:10]))
            ...
            if soup.find("ul", attrs={"class": "pager"}).find("li", attrs={"class": "next"}):
                pager_idx += 1
                category_pager_url = "{0}?p={1}".format(
                    category_url, pager_idx)
                print("{0}ページ目の{1}に進みます".format(
                    pager_idx, category_pager_url))
                driver.get(category_pager_url)
                utils.rand_sleep()
